...
/**
 * Element to map outcome sets to a course
 *
 * @package   core_outcome
 * @category  outcome
 * @copyright Copyright (c) 2013 Moodlerooms Inc. (http://www.moodlerooms.com)
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class MoodleQuickForm_map_outcome_set extends HTML_QuickForm_element {
    /**
     ...
     */
    function MoodleQuickForm_map_outcome_set($elementName=null, $value='', $attributes=null) {
        $this->HTML_QuickForm_element($elementName, $value, $attributes);
        $this->_type = 'mapoutcomeset';
    ...
    function toHtml() {
        ...
        $randomid = html_writer::random_id('outcomeset');

        $PAGE->requires->yui_module(
            'moodle-core_outcome-mapoutcomeset',
            'M.core_outcome.init_mapoutcomeset',
            array(array(
                'srcNode'   => '#'.$randomid,
    ...
    function exportValue(&$submitValues, $assoc = false) {
        ...
        // Decode and return array of filter_model instances.
        $value = $this->_findValue($submitValues);
        if (null === $value) {
            $value = $this->getValue();
        }
        if (!empty($value)) {
            /** @var $filterlist \core_outcome\model\filter_model[] */
            $filterlist = array();
            $rawvalues  = json_decode($value);
            foreach ($rawvalues as $rawvalue) {
                if (!array_key_exists($rawvalue->outcomesetid, $filterlist)) {
                    $filter = new \core_outcome\model\filter_model();
